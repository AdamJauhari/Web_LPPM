...
if ($user) {
    ...
    // Update password ke admin123, pastikan role = admin_lppm
    $hash = password_hash('admin123', PASSWORD_BCRYPT);
    $pdo->prepare("UPDATE users SET password = ?, role = 'admin_lppm' WHERE email = ?")->execute([$hash, 'admin@uca.ac.id']);
    echo "[OK] Password diperbarui ke: admin123\n";
    echo "[OK] Role dipastikan: admin_lppm\n";
} else {
    $hash = password_hash('admin123', PASSWORD_BCRYPT);
    $now = date('Y-m-d H:i:s');
    $pdo->prepare("INSERT INTO users (name, email, password, role, created_at, updated_at) VALUES (?, ?, ?, 'admin_lppm', ?, ?)")
        ->execute(['Administrator LPPM', 'admin@uca.ac.id', $hash, $now, $now]);
